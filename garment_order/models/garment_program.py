...
class GarmentProgram(models.Model):
    _name = 'garment.program'
...
#-------------------------------------------------------------    
# region (2) Danh sách liên kết 
    product_code_ids = fields.One2many('garment.style', 'garment_program_id',
    ...
    attachment_ids = fields.One2many(
        ...
        domain=[('res_model', '=', 'garment.program')],
        string="File đính kèm"
    )    
# endregion
#-------------------------------------------------------------    
# region (1) THÔNG TIN CỦA 1 CHƯƠNG TRÌNH   
...
#-------------------------------------------------------------    
# region (3 ) QUY TRÌNH PHÊ DUYỆT CHƯƠNG TRÌNH
    ...
    manager_ids = fields.Many2many('employee.base', string="Quản lý phê duyệt",
        relation='warehouse_manager_rel',
        column1='warehouse_id',
        column2='manager_id',
        help="Quản lý phê duyệt", required=True,domain="[('id', 'in', allowed_manager_ids)]")

    department_approval_id = fields.Many2many('employee.base',string="Phê duyệt mức 2",
        relation='warehouse_department_approval_rel',
        column1='warehouse_id',
        column2='department_id',
        help="Phê duyệt mức 2, có thể đồng thời nhiều người duyệt",domain="[('id', 'in', allowed_department_approval_ids)]")
    ...
    def _compute_button_visibility(self):
        for record in self:
            ...
            is_admin = self.env.user.has_group('garment_order.group_order_management_manager')
    ...
    def _get_next_state(self):
        ...
        if self.env.user.has_group('garment_order.group_order_management_manager'):
    ...
    def action_manager_order(self):
        ...
        if user_id not in self.manager_ids.mapped('user_id.id') and not self.env.user.has_group('garment_order.group_order_management_manager'):
    ...
    def action_validate(self):
        ...
        if user_id not in self.department_approval_id.mapped('user_id.id') and not self.env.user.has_group('garment_order.group_order_management_manager'):
    ...
    # Kiểm tra quyền từ chối dựa trên trạng thái hiện tại
        if self.state_order == 'manager_approval' and not self.env.user.has_group('garment_order.group_order_management_manager'):
...
# region (4) Tạo lệnh sản xuất
    ...
    def action_create_product_code(self):
        ...
        return {
            ...
            'name': 'Tạo mới Style',
            'res_model': 'garment.style',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'default_garment_program_id': self.id,
            }
        }
...
#-------------------------------------------------------------
# region (5) Phương thức create và write để thêm logic tùy chỉnh
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if vals.get('order_index', 'New') == 'New':
                vals['order_index'] = self.env['ir.sequence'].next_by_code('garment.program') or 'New'
            # 2. Set initial state
            vals['state_order'] = 'manager_approval'

        # Create the records
        approvals = super(GarmentProgram, self).create(vals_list)
    ...
    def write(self, vals):
        ...
        if 'follow_ids' in vals:
            ...
            is_manager = current_user.has_group('garment_order.group_order_management_manager')
        ...
        res = super(GarmentProgram, self).write(vals)
        ...
        return res
#------------------------------------------------------------
# region (6) PHẦN 1: Tìm kiếm, import/export danh sách Style           
