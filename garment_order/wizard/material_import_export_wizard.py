...
class MaterialImportExportWizard(models.TransientModel):
    ...
    _description = 'Wizard Import Vật Tư theo Chương trình - Style - Màu - Size'
    ...
    def action_download_template(self):
        return {
            ...
            'url': '/order_management/static/src/xlsx/import_progarm_excel/form_material_style_color_size_.xlsx',
            'target': 'self',
        }
    ...
    def import_excel(self):
        ...
        required_cols = [
            'Style#', 'Color.style#', 'Color.style Name', 'Mtr#',
            'Supplier#', 'Supplier', 'Price', 'Cif_price', 'Fob_price', 'Exwork_price'
        ]
        ...
        for idx, row in df.iterrows():
            ...
            product_code = str(row.get('Style#', '')).strip()
            color_name = str(row.get('Color.style Name', '')).strip()
            color_code = str(row.get('Color.style#', '')).strip()

            if not product_code or not color_name:
                raise ValidationError(f"Dòng {row_num_display}: Thiếu Style# hoặc Color Name.")
            ...
            if not code_rec:
                raise ValidationError(f"Dòng {row_num_display}: Không tìm thấy style '{product_code}' trong chương trình.")
        ...
        message = f"✅ Import vật tư thành công cho chương trình {self.order_id.name}."
        return {
