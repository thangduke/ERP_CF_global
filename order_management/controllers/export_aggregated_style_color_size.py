...
class AggregatedProductColorSizeExportController(http.Controller):
    ...
    @http.route('/export/aggregated_product_color_size/<int:product_code_id>', type='http', auth='user')
    def export_aggregated_product_color_size_excel(self, product_code_id, **kwargs):
        try:
            product_code = request.env['product.color.size'].sudo().browse(product_code_id)
            ...
            template_path = os.path.join(module_path, '../static/src/xlsx/export_product_color_size/export_aggregated_product_color_size.xlsx')
            if not os.path.exists(template_path):
                return request.make_response(
                    "Không tìm thấy file mẫu export_aggregated_product_color_size.xlsx.",
                    [('Content-Type', 'text/plain; charset=utf-8')]
                )
            ...
            header_data = {
                ...
                'C8': product_code.product_code_id.name or '',
                'C9': product_code.customer_id.name_customer or '',
                'F7': product_code.color or '',
                'F8': product_code.color_id.name or '',
                'F9': product_code.size_id.name or '',
                'H7': product_code.order_qty or '',
                'H8': product_code.test_qty or '',
            }
