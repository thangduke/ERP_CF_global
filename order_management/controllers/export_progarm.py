...
class ProductCodeExportController(http.Controller):
    ...
    @http.route('/export/product/<int:product_id>', type='http', auth='user')
    def export_customer_excel(self, color_size_id, **kwargs):
        # Lấy bản ghi khách hàng
        customer = request.env['product.color.size'].sudo().browse(color_size_id)
        lines = customer.material_ids

        # Xây dựng đường dẫn tuyệt đối đến file mẫu
        module_path = os.path.dirname(os.path.abspath(__file__))
        template_path = os.path.join(module_path, '../static/src/xlsx/export_progarm_customer.xlsx')
        ...
        filename = f'product_export_{current_time}.xlsx'
