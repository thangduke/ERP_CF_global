...
class MaterialReceiveExportController(http.Controller):

    @http.route('/export/material_receive/<int:receive_id>', type='http', auth='user')
    def export_material_receive_excel(self, receive_id, **kwargs):
        # Lấy bản ghi đơn nhập kho
        receive = request.env['material.receive'].sudo().browse(receive_id)
        lines = receive.material_line_ids

        # Xây dựng đường dẫn tuyệt đối đến file mẫu
        module_path = os.path.dirname(os.path.abspath(__file__))
        template_path = os.path.join(module_path, '../static/src/xlsx/export_material_receive.xlsx')
        ...
        filename = f'don_nhap_kho_{current_time}.xlsx'
